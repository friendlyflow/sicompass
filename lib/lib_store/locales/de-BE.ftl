...
store-state-data-waiting = nicht installiert, Ihre Daten dafür sind auf diesem Computer
store-state-used-before = nicht installiert, Sie haben es früher benutzt
store-data-waiting = Ihre Daten dafür sind auf diesem Computer, in { $path }. Die Installation öffnet diese Daten wieder.
store-data-waiting-root = Nicht installiert, aber Sie haben sie früher benutzt: { $names }. Sie waren bis Version 0.2 in sicompass enthalten und kommen jetzt aus dem Store. Installieren Sie sie unter Programme, um dort weiterzumachen, wo Sie aufgehört haben.
