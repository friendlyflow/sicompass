...
use crate::payments::{CLOUD_SLUG, SUPPORT_SLUG, cert, checkout, config, redeem_license};
...
impl TierSession {
    ...
    pub fn set_store_url(&mut self, url: &str) {
        self.store_url = if url.is_empty() {
            crate::payments::DEFAULT_STORE_URL.to_owned()
        } else {
            url.to_owned()
        };
    }
...
mod tests {
    ...
    fn clearing_the_store_url_falls_back_to_the_default() {
        ...
        assert_eq!(s.store_url(), crate::payments::DEFAULT_STORE_URL);
    }
}
