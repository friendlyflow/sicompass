...
//!
//! Card and column text is the most user-controlled string this provider has —
//! whatever the user typed, saved verbatim, and rendered back. A card reading
//!
//! ```text
//! remember: <button>submit</button>Send
//! ```
//!
//! would come back as a live button. So board text is escaped on its way into a
...
mod tests {
    ...
    #[test]
    fn a_card_that_looks_like_a_button_stays_text() {
        round_trip("<button>submit</button>Send");
    }

    #[test]
    fn a_card_that_looks_like_an_input_stays_text() {
        round_trip("<input>type here</input>");
    }

    #[test]
    fn a_card_that_looks_like_an_id_cannot_forge_one() {
    ...
    fn a_lone_backslash_survives_a_round_trip() {
        round_trip(r"C:\board");
        assert_eq!(unescape(r"C:\board"), r"C:\board");
    }
