...
tutorial-prog-terminal = Terminal: put the cursor on a folder and press : to turn the list into a real shell running in that folder, with an input line at the bottom. Escape returns to the folders, and full-screen programs like vim and htop get their own interactive mode.
