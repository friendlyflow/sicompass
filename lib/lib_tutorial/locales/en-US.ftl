...
tutorial-prog-web = Web browser, from the Store: type a URL in the address bar to load a page as a navigable tree of headings, links, and forms you can fill in. The cursor drops into the content once the page lands, and what it finds is grouped into navigation, main content, complementary, and footer, so you can step over a menu instead of reading through it.
tutorial-prog-web-history = Web addresses you have visited sit as rows under the address bar, newest first. Press Enter on one to load it again, and it moves back to the top. The list is saved, so it is still there the next time you start.
tutorial-prog-web-bookmark = b: bookmark the history row you are on, or the page you are currently reading, even one you reached by following a link. Bookmarked rows are announced with [bookmark] in front, and they stay in the list once older addresses are dropped to keep it within its limit. Press b again to remove the bookmark.
tutorial-prog-web-commands = Web browser commands: clear cookies signs you out of every site, and show hidden content reloads the page with nothing left out, which is the way back if a page reads oddly. A site that asks about its cookies puts that question first, as a step you answer before the content appears, and nothing is accepted on your behalf. A site offering several languages is not a question, its own language links are read as part of the page.
tutorial-prog-terminal = Terminal, from the Store: press : to turn the list into a real shell running in the folder you are in, with an input line at the bottom. The header then says command mode. Walk into a folder with Right first if you want the shell one level deeper. Escape returns to the folders, and full-screen programs like vim and htop get their own interactive mode.
tutorial-prog-claude = Claude, from the Store: press : to turn the list into a Claude session running in the folder you are in, with an input line at the bottom. The header then says first command mode, because a second : is available here. The folder matters, because it decides which project Claude can see, so walk into one with Right first if you want it working a level deeper. Type a prompt and press Enter, and the answer arrives as a tree of messages, tool calls, and results you can walk through. Escape returns to the folders. Pressing : again at the input line opens the project's skills in second command mode, where Enter drops the chosen skill into your prompt. Pressing : in a different folder starts a fresh session there, which replaces the previous conversation.
tutorial-prog-git = Git client, from the Store: browse to a folder inside a repository and press : to open it. The list then holds changes, graph, branches, stashes, and remotes, and : from there is the git commands rather than another view. Escape goes back to the folders. The top row says which repository you are in, which branch you are on, and how far ahead or behind the remote you are.
tutorial-prog-git-commit = Under changes, the first row is the commit message, so press i and type it. Below it are four buttons, commit, commit amending the last one, commit and push, and commit and sync, and Enter on one runs it. Below those is every changed file, conflicts first, then what is staged, then what is not, and Right on a file reads its diff line by line. Staging, discarding, branching, and stashing all live on the : commands, and staging and committing can be undone with ctrl z.
tutorial-prog-chat = Chat, from the Store: a Matrix client. Log in inline, your rooms appear as a tree, and you type a message and press Enter to send.
tutorial-prog-email = Email, from the Store: IMAP and SMTP with Gmail OAuth. Folders and messages form a tree, and you can compose, reply, move, and delete, all undoable.
...
tutorial-prog-salesdemo = Sales demo, from the Store: an air handling unit configurator that shows how complex, editable, hierarchical data works in Sicompass.
tutorial-prog-remote = Remote, from the Store: browse lists your own servers serve over HTTP. Add each server in Settings, under remote, as a line: name URL.
...
tutorial-ext-build = You can build your own programs. Rust is the standard way, implement the Provider trait from the sicompass-sdk crate and register it, exactly as every built-in program does. Plugins you install are WASM components instead, sandboxed so that a plugin can only reach the outside world through what the app hands it, which is why its permissions are limits rather than promises. Install them under ~/.config/sicompass/plugins/ and enable them in Settings.
tutorial-ext-docs = The full Provider trait, the element tags, and a worked example are in the repository docs and the sicompass-sdk crate, not in this tutorial. See docs/wasm-plugins.md for what a WASM plugin may and may not do, the lib/ crates for built-in Rust providers, and the sales demo plugin (salesdemo_plugin_sicompass on GitHub) for a complete plugin.
