...
tutorial-ext-docs = The full Provider trait, the element tags, and a worked example are in the repository docs and the sicompass-sdk crate, not in this tutorial. See docs/wasm-plugins.md for what a WASM plugin may and may not do, and the lib/ crates for Rust providers, for example lib/lib_sales_demo/.
