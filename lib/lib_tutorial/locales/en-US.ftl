...
tutorial-prog-terminal = Terminal: press : to turn the list into a real shell running in the folder you are in, with an input line at the bottom. Walk into a folder with Right first if you want the shell one level deeper. Escape returns to the folders, and full-screen programs like vim and htop get their own interactive mode.
tutorial-prog-claude = Claude: press : to turn the list into a Claude session running in the folder you are in, with an input line at the bottom. The folder matters, because it decides which project Claude can see, so walk into one with Right first if you want it working a level deeper. Type a prompt and press Enter, and the answer arrives as a tree of messages, tool calls, and results you can walk through. Escape returns to the folders. Pressing : in a different folder starts a fresh session there, which replaces the previous conversation.
