...
- **Tabbed Workspaces**: Each tab owns its own providers and state (its own shell and working directory, for example). Switch with a most-recently-used overlay (hold Ctrl+Tab, or open the `t` palette and type to filter), open with Ctrl+T, and close with Ctrl+W (a busy tab asks for confirmation first)
- **Keyboard-Reachable Window Controls**: A borderless custom titlebar draws its own minimize, maximize, and close controls. Reach them from the keyboard through the `c` palette, or click the titlebar buttons
